// Clawd — Claude Code usage. GNOME Shell extension, GNOME 42–44 LEGACY port.
// Mirror of gnome/extension/extension.js. Same logic; uses the pre-45 module
// system (imports.gi / Me.imports) and the legacy init/enable/disable entry
// points instead of ESM + the Extension base class.
...
const Core = Me.imports['clawd-core'];
const {
...
const { AnimationRunner } = Me.imports.anim_runner;
...
// ─── Extension entry ───
// Legacy substitute for the 45+ Extension base class: provides the same
// .uuid / .path / .getSettings() surface the indicator and methods rely on.
var ClawdExtension = class ClawdExtension {
    constructor() {
    ...
    getSettings(schemaId) {
        return ExtensionUtils.getSettings(schemaId);
    }
    ...
    _buildLockOverlay() {
        // `lockscreen-size-percent` is the fraction of monitor width Clawd's
        // body spans (10 = a tenth, default; 50 = half). Floored to ≥4 px per
        // cell so HiDPI 4K stays readable and a tiny VM window doesn't render
        // at zero pixels. Square cells (yUnit = xUnit).
        const monitor = Main.layoutManager.primaryMonitor;
        const monitorW = monitor ? monitor.width : 1920;
        const sizePercent = this._settings
            ? Math.max(5, Math.min(50, this._settings.get_int('lockscreen-size-percent')))
            : 10;
        const LOCK_XUNIT = Math.max(4, Math.floor(monitorW * sizePercent / 100 / COLS));
...
// ─── Legacy entry points (GNOME 42–44) ───
let _clawdExt = null;
