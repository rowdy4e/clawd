// Shared building blocks for the GNOME 42–44 legacy panel indicator.
// Mirror of gnome/extension/clawd-core.js — identical logic; only the module
// boilerplate differs: ESM imports/exports become legacy `imports`/`var` so
// GJS exposes the symbols as Me.imports.clawd_core.*.
// (GJS legacy modules only expose top-level `var`/`function`, never const/
// let/class — hence every `export const`/`export class` becomes `var`.)
...
function _loadJSON(filename) {
    // Read a sibling file from the extension dir via Gio (TextDecoder turns
    // bytes into a string). Me.path is the installed extension directory.
    const file = Gio.File.new_for_path(`${Me.path}/${filename}`);
...
var COLS = _SHARED_FORMS.grid.cols;
var ROWS = _SHARED_FORMS.grid.rows;
var FORMS = _SHARED_FORMS.forms;
var FORM_KEYS = Object.keys(FORMS);
var RAINBOW = (_SHARED_FORMS.palettes && _SHARED_FORMS.palettes.rainbow) || [];
var ANIMATIONS_JSON = _SHARED_ANIMS;
...
var MORPH_TARGETS = FORM_KEYS.filter(k =>
    k !== 'clawd' && _formAllowsContext(FORMS[k], 'panel'));
var EASTER_EGGS = FORM_KEYS.filter(k => false);  // unused — animations.json tags drive easter-egg roll instead
...
function drawClawd(Cairo, cr, w, h, s, xUnit, yUnit) {
...
// ─── Easing functions ────────────────────────────────────────────────────
var easeOutQuad   = t => 1 - (1 - t) ** 2;
var easeInQuad    = t => t * t;
var easeInOutQuad = t => t < 0.5 ? 2*t*t : 1 - ((-2*t+2) ** 2) / 2;
var easeInOutCubic = t => t < 0.5 ? 4*t*t*t : 1 - ((-2*t+2) ** 3) / 2;
var easeOutBack   = t => 1 + 2.70158 * (t-1)**3 + 1.70158 * (t-1)**2;
var easeOutBounce = t => {
